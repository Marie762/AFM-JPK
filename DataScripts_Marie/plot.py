...
def Fd1(F, d, contact_point_list, save=False):
    for k in range(len(F)):
        ...
        ax.plot(d[k][0][contact_point_list[k]], F[k][0][contact_point_list[k]], 'ro', markersize=8, label='estimated contact point')
        ax.set_xlabel(u'Distance (\u03bcm)', fontsize=15)
        ax.set_ylabel('Force (nN)', fontsize=15)
        plt.legend(loc="upper right", prop={'size': 15})
        plt.tick_params(axis='both', which='major', labelsize=15)
        ...
        if k ==0:
            fig.savefig('Results\Fd_contact_point_' + str(k) + '.pdf', format='pdf')
        plt.close()
    return 
...
def FdGrid_Height(data, x_position, y_position, k, save=False, name = '_', interpolation='gaussian'):
    ...
    fig.colorbar(im, ax=ax, label=u'Height (\u03bcm)') # height map
    ...
    if save:
        ...
        fig.savefig('Results\AFdGrid_real_contact_point_' + name + str(k) + '.pdf', format='pdf')
    return fig
...
def FdGrid_Peaks(data, x_position, y_position, k, save=False, name = '_', interpolation='gaussian'):
    ...
    if save:
        ...
        fig.savefig('Results\AFdGrid_' + name + str(k) + '.pdf', format='pdf')
    return fig
...
def FdGrid_Indentation(data, x_position, y_position, k, save=False, name = '_', interpolation='gaussian'):
    ...
    im = ax.imshow(dataframe_qmap, origin='lower', extent=(y_position[0], y_position[-1], x_position[0], x_position[-1]), 
                   cmap='hot_r', interpolation=interpolation)  #, vmin = 0, vmax = 5, vmin = 8.75, vmax = 10.5, interpolation='gaussian', cmap='Blues_r' or 'hot_r'
    fig.colorbar(im, ax=ax, label=u'Indentation depth (\u03bcm)') # indentation depth map
    ax.set_xlabel(u'x (\u03bcm)', fontsize=15)
    ax.set_ylabel(u'y (\u03bcm)', fontsize=15)
    plt.tick_params(axis='both', which='major', labelsize=15)
    if save:
...
def FdGrid_Emodulus(data, x_position, y_position, k, save=False, name = '_', interpolation='gaussian'):
    ...
    fig.colorbar(im, ax=ax, label='Elastic modulus (kPa)') # Youngs modulus map
    ax.set_xlabel(u'x (\u03bcm)', fontsize=15)
    ax.set_ylabel(u'y (\u03bcm)', fontsize=15)
    plt.tick_params(axis='both', which='major', labelsize=15)
    if save:
